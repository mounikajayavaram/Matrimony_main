...
# env = environ.Env()
from Membershipapi.models import *
...
accesskey="01e05f30-a2df-11ed-a3c3-0bbe2da489ce"
secretkey="5108d0a10f06c4f91175d3f137578f081a5c3f91"
environment="test"
...
def callback(request):
	...
	if status=="Transaction Successful" and s.paymentStatus != "success":
		...
		json1_file = open('paymentapi/json/pricingPlan.json','r+', encoding="utf-8")
		...
		# title=response["layer_payment_id"]
		...
		# x=All_Courses.objects.create(user=user,title=title,image=image,logo=logo,badge=badge)
		# x.save()
		s.save()
		finalData["course"]=s.courseSelected.pk
		finalData["categoryChoose"]=s.courseCategoryPK.pk

		# finalData["title"]=title
		# finalData["image"]=image
		# finalData["logo"]=logo
		# finalData["badge"]=badge
	...
	# data=serializers.serialize('json',finalData)
...
class create_payment_token_Class(APIView):
	def post( self,request , format=None):
		...
		# coursePK=request.data.get("courseSelectPK")
		courseCategoryPK=request.data.get("courseCategoryPK")
		PlandataPK=Plan.objects.get(id=courseCategoryPK)
		...
		# else:
		print("trainingCourseForm,",trainingCourseForm)
		data["amount"] = str(PlandataPK.price)+".00"
		...
		if error == "":
			...
			layer_params = "{payment_token_id:"+payment_token_data["id"]+",accesskey:"+accesskey+"}"
			token_id=payment_token_data["id"]
		print("coursePK",coursePK,courseCategoryPK)
		s=TransactionDetails.objects.create(payment_token_id=token_id,payment_id="",userID_id=name,courseSelected=All_Courses.objects.get(pk=int(coursePK)),paymentStatus="Canceled",courseCategoryPK=trainingCourseForm.objects.get(pk=courseCategoryPK),email=data["email_id"],tranid=data["mtx"],order_payment_amount=data["amount"],phoneNumber=data["contact_number"])
